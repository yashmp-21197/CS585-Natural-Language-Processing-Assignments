...
class NaiveBayes:
    ...
    def train(self, data):
        ...
        for i, text in enumerate(data.get_texts("pos")):
            ...
            #
            # increase number of positive text's count by 1
            self.num_positive_reviews += 1
            # iterate all words in text file
            for word in text.get_words():
                # increase total positive words by 1
                self.total_positive_words += 1
                # increase positive count of that word by 1
                self.count_positive[word] = self.count_positive[word] + 1
            if i % 100 == 0:
                sys.stdout.write(".")
        print()
        for i, text in enumerate(data.get_texts("neg")):
            ...
            # Aggregate counts here
            #
            # increase number of negative text's count by 1
            self.num_negative_reviews += 1
            # iterate all words in text file
            for word in text.get_words():
                # increase total negative words by 1
                self.total_negative_words += 1
                # increase negative count of that word by 1
                self.count_negative[word] = self.count_negative[word] + 1
            if i % 100 == 0:
                sys.stdout.write(".")
        print()
        # End TODO
    ...
    def predict(self, data, alpha=1.0):
        ...
        for classval in ["pos", "neg"]:
            for text in data.get_texts(classval):
                ...
                # iterate all words in text file
                for word in text.get_words():
                    # increase sum_positive by logarithmic probability of that word with adding smoothing parameter alpha in the counts of that word.
                    sum_positive += log(float(self.count_positive[word] + alpha) / float(self.total_positive_words + (alpha * len(self.vocab))))
                    # increase sum_negative by logarithmic probability of that word with adding smoothing parameter alpha in the counts of that word.
                    sum_negative += log(float(self.count_negative[word] + alpha) / float(self.total_negative_words + (alpha * len(self.vocab))))
                # add logarithmic probability of text being positive into sum_positive
                sum_positive += self.p_positive
                # add logarithmic probability of text being negative into sum_negative
