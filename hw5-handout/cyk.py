    ...
    for m in range(1, len(words)):
        for n in range(0, len(words)-m):
            ...
            for k in range(n+1, n+m+1):
                for ntr in nt_rules:
                    if ntr.rhs1 in chart[k-n-1][n] and ntr.rhs2 in chart[m+n-k][k]:
                        chart[m][n][ntr.lhs] = ParsedPhrase(label=ntr.lhs, children=[chart[k-n-1][n][ntr.rhs1], chart[m+n-k][k][ntr.rhs2]])

    for row in range(0, len(chart)):
        for col in range(0, len(chart[row])):
            print(row, col)
    #         # for m in chart[row][col].keys():
    #         #     print(m)
            print(chart[row][col])
            print()
    print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grammar_file", default="grammar_3.txt")
    parser.add_argument("--sentence_file", default="sentences_3.txt")
